...
var path_start_position = Vector2() :
	get:
		return path_start_position # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of _set_path_start_position
var path_end_position = Vector2() :
	get:
		return path_end_position # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of _set_path_end_position
...
@onready var _half_cell_size = cell_quadrant_size / 2
...
func _process(_delta):
	if nav_map_up_to_date==false:
		...
		var walkable_cells_list = astar_add_walkable_cells(get_used_cells(0))
...
##		var pos=map_to_local(astar_node.get_point_position(i))
...
func update_navigation_map():
	...
	shape.extents=(cell_quadrant_size*0.75)
	...
	for point_x in map_size.x:
		for point_y in map_size.y:
			var space = get_world_2d().direct_space_state
			var point_position=map_to_local(Vector2(point_x,point_y))
			query.set_transform(Transform2D(0, point_position+(cell_quadrant_size*0.75 )))
	...
	print("calculation time: "+str(Time.get_ticks_msec()-time_start)+"ms")
...
#	var end_tile = local_to_map(end.round())
#
...
#		var point_world = map_to_local(point)+half_cell_size
#		path_world.append(point_world)
#
#	return path_world
