...
func destroy_window():
	if destroyed==false:
		...
		play_audio(destruction_sounds[rand_range(0,destruction_sounds.size()-1)])
		destroyed=true
...
func play_audio(given_sample,affected_time=true,true_pitch=1,random_pitch=0,bus="Master"):
	if (given_sample is String):
		...
		if (typeof(given_sample[0])==1 && given_sample[0]==false):
			play_audio(given_sample[rand_range(1,given_sample.size()-1)],affected_time,true_pitch,random_pitch,bus)
		else:
