...
@onready var col_check=get_node("Area2D")

func _ready():
	get_node("Sprite").speed_scale=0
...
func destroy_window():
	if destroyed==false:
		...
		play_audio(destruction_sounds[randi_range(0,destruction_sounds.size()-1)])
		destroyed=true


func window_collision_check():
	var return_array=[]
	for i in col_check.get_overlapping_bodies():
		if (i is BULLET or i is WEAPON):
...
func play_audio(given_sample,affected_time=true,true_pitch=1,random_pitch=0,bus="Master"):
	if (given_sample is String):
		...
		get_viewport().call_deferred("add_child",audio_player)
		...
		if (typeof(given_sample[0])==1 && given_sample[0]==false):
			play_audio(given_sample[randi_range(1,given_sample.size()-1)],affected_time,true_pitch,random_pitch,bus)
		else:
