...
var bullets=rand_range(120,160)
var list_of_bullets
var climb=bullets
		...
		if a10_node.position.x>-1000:
			if bullets>0:
				...
				if spawn_bullet_timer<=0:
					get_tree().get_nodes_in_group("Camera")[0].shake=30
					...
					bullet_instance.global_position=global_position+Vector2(a10_node.position.x+300+(climb-bullets)*10,rand_range(-100,100)).rotated(global_rotation)
					bullet_instance.speed=2200
					bullet_instance.bullet_height=0.05+(climb-bullets)*0.003
					bullet_instance.global_rotation=global_rotation+deg2rad(rand_range(-5,5))
...
#					bullet_instance.global_rotation=-(a10_node.global_position.direction_to(bullet_instance.global_position)).angle()
		...
		if bullets>=get_node("a10/bullet_cache").get_child_count():
			if spawn_bullet_timer<=0:
				var bullet_instance=a10_bullet.instance()
				bullet_instance.connect("bullet_signal",self,"confirm_kill")
